function convertNotes() {
    ...
    let input = document.getElementById("inputPianoNotes").value;
    ...
    notesArray.forEach((note) => {
        let index = p.indexOf(note);
        if (index !== -1) {
            convertedNotes.push(g[index]);
        }
    });

    out.innerHTML = "<br>Corresponding Guitar Tabs: <br><br>" + convertedNotes.join("   ");
}